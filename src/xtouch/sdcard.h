...
bool xtouch_sdcard_setup()
{
    ...
    if (!xtouch_filesystem_exist(SPIFFS, xtouch_paths_config))
    {
        lv_label_set_text(introScreenCaption, LV_SYMBOL_USB "正在写入配置");
        lv_obj_set_style_text_color(introScreenCaption, lv_color_hex(0xFF0000), LV_PART_MAIN | LV_STATE_DEFAULT);
        lv_timer_handler();

        xtouch_filesystem_mkdir(SPIFFS, xtouch_paths_root);
        
        File configFile = SPIFFS.open(xtouch_paths_config, FILE_WRITE);
        StaticJsonDocument<1024> config;
        config["ssid"] = "88888888";
        config["pwd"] = "88888888";
        config["cloud-email"] = "88888888";
        config["cloud-password"] = "88888888";
        config["user-id"] = "88888888";
        config["cloud-region"] = "China";
